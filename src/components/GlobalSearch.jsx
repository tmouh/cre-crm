...
export default function GlobalSearch() {
  ...
  const results = useMemo(() => {
    ...
    properties.filter(p => p.name?.toLowerCase().includes(q) || p.address?.toLowerCase().includes(q))
      .slice(0, 5).forEach(p => items.push({ type: 'deal', id: p.id, title: p.name || p.address, subtitle: [formatDealType(p.dealType), formatDealStatus(p.status), formatCurrency(p.dealValue)].filter(Boolean).join(' · '), icon: Briefcase, to: `/properties/${p.id}` }))
  ...
  if (!open) {
    return (
      <button onClick={() => { setOpen(true); setTimeout(() => inputRef.current?.focus(), 50) }}
        className="flex items-center gap-2 px-3 py-1.5 text-sm text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors border border-gray-200 dark:border-gray-700">
        <Search size={14} />
        <span className="hidden sm:inline">Search...</span>
        <kbd className="hidden sm:inline text-[10px] px-1.5 py-0.5 bg-gray-200 dark:bg-gray-700 rounded text-gray-400 dark:text-gray-500 font-mono">Ctrl+K</kbd>
      </button>
    )
  }
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 dark:border-gray-700">
          ...
          <input
            ...
            autoFocus
          />
          <button onClick={() => { setOpen(false); setQuery('') }} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
            <X size={16} />
          </button>
        </div>
          ...
          <div className="max-h-80 overflow-y-auto py-1">
            {results.map((r, i) => (
              <button
                ...
                className={clsx('w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors', i === selectedIdx ? 'bg-brand-50 dark:bg-brand-900/20' : 'hover:bg-gray-50 dark:hover:bg-gray-700/50')}
              >
                <r.icon size={15} className="text-gray-400 dark:text-gray-500 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{r.title}</p>
                  {r.subtitle && <p className="text-[11px] text-gray-400 dark:text-gray-500 truncate">{r.subtitle}</p>}
                </div>
                <span className="text-[10px] text-gray-400 dark:text-gray-500 uppercase">{r.type}</span>
              </button>
            ))}
          </div>
        )}

        <div className="px-4 py-2 border-t border-gray-100 dark:border-gray-700 flex items-center gap-4 text-[10px] text-gray-400 dark:text-gray-500">
