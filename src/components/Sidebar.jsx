    ...
    <aside className="w-[220px] flex-shrink-0 bg-white dark:bg-gray-900 border-r border-gray-200/80 dark:border-gray-700/80 flex flex-col h-screen sticky top-0">
                    ...
                    {children.map(child => (
                      <NavLink
                        ...
                        className={({ isActive }) => clsx(
                          ...
                          isActive
                            ? 'bg-brand-50 text-brand-700 dark:bg-brand-900/30 dark:text-brand-300'
                            : 'text-gray-400 hover:bg-gray-50 hover:text-gray-700 dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300'
                        )}
                      >
                        {({ isActive }) => (
                          <>
                            <child.Icon size={14} strokeWidth={isActive ? 2 : 1.5} className={clsx('flex-shrink-0', isActive ? 'text-brand-600 dark:text-brand-400' : 'text-gray-400 group-hover:text-gray-500 dark:text-gray-500 dark:group-hover:text-gray-400')} />
      ...
      <div className="px-3 pb-2">
        ...
        >
          {({ isActive }) => (
            <>
              <Settings size={16} strokeWidth={isActive ? 2 : 1.5} className={clsx('flex-shrink-0', isActive ? 'text-brand-600 dark:text-brand-400' : 'text-gray-400 group-hover:text-gray-600 dark:text-gray-500 dark:group-hover:text-gray-300')} />
              <span>Settings</span>
            </>
          )}
        </NavLink>
      ...
      <div className="px-4 py-3.5 border-t border-gray-100 dark:border-gray-800 space-y-2">
        ...
        <button
          onClick={signOut}
          className="flex items-center gap-2 w-full px-2 py-1.5 text-[11px] text-gray-400 hover:text-red-500 rounded-lg hover:bg-red-50 dark:text-gray-500 dark:hover:bg-red-900/20 transition-colors"
        >
