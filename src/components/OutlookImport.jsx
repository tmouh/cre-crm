    ...
    for (let i = 0; i < toImport.length; i++) {
      ...
      try {
        ...
        newContact = await addContact({
          ...
          sharedEmails:     importVisibility === 'shared'  ? allEmails : [],
          personalPhones:   importVisibility === 'private' ? [oc.businessPhones?.[0], oc.mobilePhone].filter(Boolean) : [],
          sharedCellPhones: importVisibility === 'shared'  ? [oc.businessPhones?.[0], oc.mobilePhone].filter(Boolean) : [],
          homePhone:         oc.homePhones?.[0] || '',
          homePhone2:        oc.homePhones?.[1] || '',
          businessPhone2:    oc.businessPhones?.[1] || '',
          businessStreet:    oc.businessAddress?.street || '',
          businessCity:      oc.businessAddress?.city || '',
          businessState:     oc.businessAddress?.state || '',
          businessPostalCode:oc.businessAddress?.postalCode || '',
          businessCountry:   oc.businessAddress?.countryOrRegion || '',
          homeStreet:        oc.homeAddress?.street || '',
          homeCity:          oc.homeAddress?.city || '',
          homeState:         oc.homeAddress?.state || '',
          homePostalCode:    oc.homeAddress?.postalCode || '',
          homeCountry:       oc.homeAddress?.countryOrRegion || '',
          otherStreet:       oc.otherAddress?.street || '',
          otherCity:         oc.otherAddress?.city || '',
          otherState:        oc.otherAddress?.state || '',
          otherPostalCode:   oc.otherAddress?.postalCode || '',
          otherCountry:      oc.otherAddress?.countryOrRegion || '',
          webPage:    oc.businessHomePage || '',
